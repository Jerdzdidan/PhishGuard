...
@else
    <!-- Aggregated Scenario Difficulty -->
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <div>
                        <h5 class="mb-0">All Scenarios Difficulty Analysis (Aggregated)</h5>
                        <p class="text-muted mb-0">Success rates across all simulation scenarios</p>
                    </div>
                    <span class="badge bg-label-primary">{{ count($aggregatedScenarios) }} Scenarios</span>
                </div>
                <div class="card-body">
                    @if(count($aggregatedScenarios) > 0)
                        <div id="aggregatedScenarioChart" class="mb-4"></div>
                            ...
                            <div class="col-md-3">
                                <div class="card bg-success text-white">
                                    <div class="card-body text-center">
                                        ...
                                        <h3 class="text-white">{{ collect($aggregatedScenarios)->where('difficulty_level', 'Easy')->count() }}</h3>
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div class="card bg-info text-white">
                                    <div class="card-body text-center">
                                        ...
                                        <h3 class="text-white">{{ collect($aggregatedScenarios)->where('difficulty_level', 'Medium')->count() }}</h3>
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div class="card bg-warning text-white">
                                    <div class="card-body text-center">
                                        ...
                                        <h3 class="text-white">{{ collect($aggregatedScenarios)->where('difficulty_level', 'Hard')->count() }}</h3>
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div class="card bg-danger text-white">
                                    <div class="card-body text-center">
                                        ...
                                        <h3 class="text-white">{{ collect($aggregatedScenarios)->where('difficulty_level', 'Very Hard')->count() }}</h3>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover" id="aggregatedTable">
                                ...
                                <tbody>
                                    @foreach($aggregatedScenarios as $scenario)
                                        <tr class="scenario-card {{ strtolower(str_replace(' ', '-', $scenario['difficulty_level'])) }}">
                                            <td>{{ $scenario['scenario'] }}</td>
                                            <td>
                                                <div class="d-flex align-items-center">
                    ...
                    @else
                        <div class="text-center py-5">
                            <i class="ri-game-line ri-3x text-muted mb-3"></i>
                            <h6 class="text-muted">No simulation data available yet</h6>
                        </div>
                    @endif
                </div>
            </div>
...
$(document).ready(function() {
    ...
    @if($lessonId && count($scenarioDifficulty) > 0)
    ...
    @if(!$lessonId && count($aggregatedScenarios) > 0)
        // Aggregated scenarios chart - show top 20 hardest
        const aggregatedData = @json($aggregatedScenarios);
        const topHardest = aggregatedData.slice(0, 20);
        
        const aggregatedChart = new ApexCharts(document.querySelector("#aggregatedScenarioChart"), {
            series: [{
                name: 'Success Rate',
                data: topHardest.map(s => s.success_rate)
            }],
            chart: {
                type: 'bar',
                height: 500,
            ...
            xaxis: {
                categories: topHardest.map((s, i) => `S${i + 1}`),
                title: {
                    text: 'Success Rate (%)'
                },
                max: 100
            },
            colors: topHardest.map(s => {
                if (s.success_rate >= 80) return '#28a745';
                if (s.success_rate >= 60) return '#17a2b8';
                if (s.success_rate >= 40) return '#ffc107';
                return '#dc3545';
